...
class TrainConfig:
    ...
    env_name: str = "mujoco/hopper/medium-v0"
    seed: int = 42
...
def train(config: TrainConfig):
    ...
    try:
        ...
        if env_spec is not None:
            ...
            # Fallback: extract base name from config.env_name
            base_name = config.env_name.split("-")[0]
        ...
        base_name = config.env_name.split("-medium")[0].split("-expert")[0].split("-random")[0].split("-replay")[0]
        env = gym.make(f"{base_name}-v0")
        eval_env = gym.make(f"{base_name}-v0")
    ...
    log_freq = max(100, config.eval_frequency // 10)  # Log progress every 10% of eval_freq

    print("=" * 70)
    print(f"Training AWAC (Finetune)")
    print("=" * 70)
    print(f"Environment: {config.env_name}")
    print(f"Seed: {config.seed}, Eval Seed: {config.eval_seed}")
    ...
    print(f"Evaluation frequency: {config.eval_frequency:,} steps")
    print("=" * 70)
    print("\nStarting training...")
    print("-" * 70)
    ...
    for t in trange(
        ...
        if (t + 1) % log_freq == 0:
            ...
            print(f"  Actor Loss: {update_result.get('actor_loss', 0):.4f} | "
                  f"Critic Loss: {update_result.get('critic_loss', 0):.4f}")
            if t >= config.offline_iterations:
        ...
        if (t + 1) % config.eval_frequency == 0:
            ...
            wandb.log({"eval/eval_score": eval_score}, step=t)
            # Minari doesn't have get_normalized_score, so we use raw score
            ...
            full_normalized_eval_scores.append(normalized_eval_score)
            
            eval_time = time.time() - eval_start_time
            
            print(f"Evaluation Results ({config.n_test_episodes} episodes, {eval_time:.1f}s):")
            ...
            if len(full_normalized_eval_scores) > 1:
                print(f"  Best Score So Far: {max(full_normalized_eval_scores):.3f}")
                print(f"  Improvement: {full_normalized_eval_scores[-1] - full_normalized_eval_scores[0]:.3f}")
            print("=" * 70)
    ...
    if full_normalized_eval_scores:
        print(f"\nFinal Evaluation Score: {full_normalized_eval_scores[-1]:.3f}")
        print(f"Best Evaluation Score: {max(full_normalized_eval_scores):.3f}")
        print(f"Average Evaluation Score: {sum(full_normalized_eval_scores)/len(full_normalized_eval_scores):.3f}")
    print("=" * 70)
