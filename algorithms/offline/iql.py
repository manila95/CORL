...
def train(config: TrainConfig):
    ...
    print(f"VF LR: {config.vf_lr}, QF LR: {config.qf_lr}, Actor LR: {config.actor_lr}")
    print("=" * 70)
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % log_freq == 0:
            ...
            print(f"  Policy Loss: {log_dict.get('policy_loss', 0):.4f} | "
                  f"Q Loss: {log_dict.get('q_loss', 0):.4f} | "
                  f"V Loss: {log_dict.get('v_loss', 0):.4f}")
            print(f"  Avg Q: {log_dict.get('q_value', 0):.3f} | "
                  f"Avg V: {log_dict.get('v_value', 0):.3f} | "
                  f"Advantage: {log_dict.get('advantage', 0):.3f}")
