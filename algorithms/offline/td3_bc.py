...
def train(config: TrainConfig):
    ...
    print(f"Alpha: {config.alpha}, Tau: {config.tau}")
    print(f"Policy noise: {config.policy_noise}, Noise clip: {config.noise_clip}")
    print("=" * 70)
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % log_freq == 0:
            ...
            print(f"  Actor Loss: {log_dict.get('actor_loss', 0):.4f} | "
                  f"Critic1 Loss: {log_dict.get('critic_loss', 0):.4f} | "
                  f"Critic2 Loss: {log_dict.get('critic2_loss', 0):.4f}")
            print(f"  Q1 Value: {log_dict.get('q1', 0):.3f} | "
                  f"Q2 Value: {log_dict.get('q2', 0):.3f} | "
                  f"BC Loss: {log_dict.get('bc_loss', 0):.4f}")
